...
def check_gradient(f, x, delta=1e-5, tol = 1e-4):
    ...
    assert analytic_grad.shape == x.shape, "Analytic gradient shape not equal to x shape"
    ...
    while not it.finished:
        ...
        xminusdelta = x.copy()
        xminusdelta[ix] -= delta
        
        fplush = f(xplusdelta)[0]
        fminush = f(xminusdelta)[0]
        
        twoh = 2 * delta 
        numeric_grad_at_ix = (fplush - fminush)/twoh
